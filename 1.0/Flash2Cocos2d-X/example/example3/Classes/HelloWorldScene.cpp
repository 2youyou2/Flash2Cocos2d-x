...
void HelloWorld::squat() {
	isSquat = true;
	armature->getAnimation()->playTo("squat", 8);
}
...
void HelloWorld::updateMovement(){
	...
	if (moveDir == STAND) {
		speedX = 0;
		armature->getAnimation()->playTo("stand", 8, 60, true, 2);
		...
		if (moveDir * face > 0) {
			speedX = 5* face;
			armature->getAnimation()->playTo("run", 8, 60, true, 2);
		}else {
			speedX = -3 * face;
			armature->getAnimation()->playTo("runBack", 4, 60, true, 2);
		}
	}
}
...
void HelloWorld::updateSpeed(){
	...
	if (isJumping) {
		if (speedY == 1 ) {
			armature->getAnimation()->playTo("fall", 8);
		}
		speedY -= 1;
	}
	if (speedX != 0) {
...
void HelloWorld::updateWeapon(){
	...
	CCBone *_body = armature->getBone("body");
	_body->getNode()->rotation=_r*0.25;
	
	CCBone *_chest = armature->getBone("chest");
	_chest->getNode()->rotation=_r*0.25;
